...
u8 efuse_one_byte_read(_adapter *padapter, u16 addr, u8 *data)
{
	...
	write32(padapter,   REG_EFUSE_CTRL, read32(padapter,  REG_EFUSE_CTRL)&( ~EF_FLAG));//read cmd
...
static u8 efuse_one_byte_write(_adapter *padapter, u16 addr, u8 data)
{
	...
	// use default program time 5000 ns
	write32(padapter,REG_EFUSE_CTRL, read32(padapter,REG_EFUSE_CTRL)|EF_FLAG);//write cmd	
