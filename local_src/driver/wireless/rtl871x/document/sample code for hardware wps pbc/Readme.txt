...
The Realtek WiFi 8712 Linux driver is able to detect the hardware button status. First of all, the customer has 
to develop an application to capture the signal sent from 8712 driver. In this package, we had provided a sample 
code named "signal_handle_ex.c" for this application. In the main function of this sample code, it will pass its 
pid (process id) to WiFi 8712 driver so that the driver will know the target process which it wants to send 
the signal when the hardware button is pressed.
...
===================================================================================================================
Note: The WiFi 8712 driver will go to check the hardware button status per 2 seconds. So, we suggest the hardware 
