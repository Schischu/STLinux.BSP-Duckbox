                                ...
                                ULONG                  *size)
{
  ...
  DEBUGF2 (2, ("%s: %s: finding firmware for '%lu%c%lu@%lu flag: %s'\n",
               __PRETTY_FUNCTION__, m_AWGPrefix,
  ...
  for (int i = 0;
    ...
    {
      s = (p[12] << 8 | p[13]) * 4;
      bool fw_isInterlaced = !!p[0];
      bool mode_isInterlaced = !!(pMode->ModeParams.ScanType == SCAN_I);

      if (mode_isInterlaced != fw_isInterlaced)
        {
          /* scan type doesn't match */
          DEBUGF2 (9, ("%s: scan type doesn't match @ %d (%u vs %u)\n",
                       m_AWGPrefix, i, pMode->ModeParams.ScanType, p[0]));
          continue;
        }
      ...
      if ((suffix && !p[1])
        ...
        {
          /* flags don't match */
          DEBUGF2 (9, ("%s: flags don't match @ %x (%s vs 0x%x)\n",
                       m_AWGPrefix, i, suffix ? suffix : "0", p[1]));
          continue;
        }
      ...
      if (pMode->ModeParams.ActiveAreaHeight != (ULONG) (p[2] << 8 | p[3]))
        {
          ...
          DEBUGF2 (9, ("%s: active area doesn't match @ %d (%lu vs %lu)\n",
                       m_AWGPrefix, i, pMode->ModeParams.ActiveAreaHeight,
                       (ULONG) (p[2] << 8 | p[3])));
          continue;
        }
      ...
      if (pMode->ModeParams.FrameRate != (ULONG) (p[4] << 24
                                                  ...
                                                  | p[7] <<  0))
        {
          ...
          DEBUGF2 (9, ("%s: frame rate doesn't match @ %d (%lu vs %lu)\n",
                       m_AWGPrefix, i, pMode->ModeParams.FrameRate,
      ...
      if ((pMode->TimingParams.ulPixelClock / 1000) != (ULONG) (p[8] << 24
                                                                ...
                                                                | p[11] <<  0))
        {
          ...
          DEBUGF2 (9, ("%s: pixel clock doesn't match @ %d (%lu vs %lu)\n",
                       m_AWGPrefix, i, pMode->TimingParams.ulPixelClock / 1000,
                       (ULONG) (p[8] << 24 | p[ 9] << 16 | p[10] <<  8 | p[11] <<  0)));
          continue;
        }

      DEBUGF2 (3, ("%s: %s: found a match @ %d, size %lu\n",
                   __PRETTY_FUNCTION__, m_AWGPrefix, i, s));
  ...
  if (LIKELY (!g_pIOS->RequestFirmware (&m_FWbundle, name)))
    {
      ...
      if (UNLIKELY (m_FWbundle->pData[0] != FIRMWARE_BUNDLE_VERSION))
        {
          DEBUGF (("%s: %s: wrong AWG FW bundle version %d, want %d\n",
                   __PRETTY_FUNCTION__, name,
      ...
      if (UNLIKELY (!m_FWbundle->pData[1]))
        {
          DEBUGF (("%s: %s: empty AWG FW bundle?!\n", __PRETTY_FUNCTION__,
                   name));
      ...
      for (int i = 0; i < m_FWbundle->pData[1]; i++)
        {
          ...
          if (UNLIKELY (size > m_maxTimingSize))
            {
              DEBUGF (("%s: %s: AWG FW bundle corrupted (1) (%hu > %d)!\n",
                       __PRETTY_FUNCTION__, name, size, m_maxTimingSize));
              goto error;
            }
          ...
          if (UNLIKELY (m_FWbundle->ulDataSize < total_size + size + 14))
            {
              DEBUGF (("%s: %s: AWG FW bundle corrupted (2)!\n",
                       __PRETTY_FUNCTION__, name));
              goto error;
            }

          DEBUGF2 (3, ("%s: %s: @ %x: %lu%c%lu@%lu flag: %c\n",
                       __PRETTY_FUNCTION__, name,
                       p - m_FWbundle->pData,
                       (ULONG) (p[2] << 8 | p[3]),
                       p[0] ? 'i' : 'p',
      ...
      if (UNLIKELY (m_FWbundle->ulDataSize != total_size))
        {
          DEBUGF (("%s: %s: AWG FW bundle corrupted (3)!\n",
                   __PRETTY_FUNCTION__, name));
          goto error;
        }
      ...
      DEBUGF2 (2, ("%s: %s: found timings for %d modes\n",
                   __PRETTY_FUNCTION__, name, n_timings_found));
#endif
    }
