...
void CSTmDENC::SetControl(stm_output_control_t ctrl, ULONG ulNewVal)
{
  switch(ctrl)
  {
    ...
    case STM_CTRL_CVBS_TRAP_FILTER:
    {
    ...
    case STM_CTRL_CHROMA_SCALE:
    {
      if(ulNewVal < m_C_MultiplyMin)
        ulNewVal = m_C_MultiplyMin;

      if(ulNewVal > m_C_MultiplyMax)
        ulNewVal = m_C_MultiplyMax;

      ulNewVal -= m_C_MultiplyMin;

      m_C_Multiply = (ulNewVal+(m_C_MultiplyStep/2)) / m_C_MultiplyStep;
      break;
    }
    default:
      break;
...
ULONG CSTmDENC::GetControl(stm_output_control_t ctrl) const
{
  ...
  switch(ctrl)
  {
    ...
    case STM_CTRL_TELETEXT_SYSTEM:
      ...
      break;
    case STM_CTRL_CHROMA_SCALE:
      ulVal = m_C_MultiplyMin + ((ULONG)m_C_Multiply * m_C_MultiplyStep);
      break;
    default:
      break;
  }
