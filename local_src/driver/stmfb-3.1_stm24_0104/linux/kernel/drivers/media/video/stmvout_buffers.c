                               ...
                               struct v4l2_requestbuffers *req)
{
  ...
  for (i = 0; i < req->count; ++i)
  {
    ...
    if (pDev->streamBuffers[i].physicalAddr != 0)
    {
      ...
      writeback_ioremap_region(pDev->streamBuffers[i].physicalAddr,
             pDev->streamBuffers[i].cpuAddr, 0, bufLen);
                      ...
                      struct v4l2_buffer *pVidBuf)
{
  ...
  else // if (pVidBuf->memory == V4L2_MEMORY_MMAP)
  {
    ...
    hdr = &pBuf->bufferHeader;
    writeback_ioremap_region(pBuf->physicalAddr, pBuf->cpuAddr,
...
#endif
        writeback_ioremap_region(pBuf->physicalAddr, pBuf->cpuAddr,
                                 pBuf->clut_virt - pBuf->cpuAddr, clutSize * 4);
      }
      break;
