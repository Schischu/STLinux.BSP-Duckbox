...
#define GET_STATE(ctrl, old_state, new_state) \
  ...
  GET_STATE (PLANE_CTRL_COLOR_KEY,
             device->state.old_ckey, device->state.new_ckey);
                   ...
                   struct v4l2_queryctrl * const pqc)
{
  ...
  case _deprecated_V4L2_CID_STM_CKEY_MAXVAL:
    {
      ...
      stm_v4l2_ctrl_query_fill (pqc,
                                name_map[pqc->id - _deprecated_V4L2_CID_STM_CKEY_MINVAL],
  ...
  case V4L2_CID_STM_CKEY_RCr_MINVAL ... V4L2_CID_STM_CKEY_BCb_MAXVAL:
    {
      static const char * const name_map[] = { "R/Cr ColorKey minval",
      ...
      stm_v4l2_ctrl_query_fill (pqc,
                                name_map[pqc->id - V4L2_CID_STM_CKEY_RCr_MINVAL],
                                0, 255, 1, 0, V4L2_CTRL_TYPE_INTEGER,
                                0);
    }
    break;
                     ...
                     const struct v4l2_ext_control * const ctrl)
{
  ...
  case V4L2_CID_STM_CKEY_ENABLE:
    ...
    state->new_ckey.enable = ctrl->value;
    break;

  case V4L2_CID_STM_CKEY_FORMAT:
    state->new_ckey.flags |= SCKCF_FORMAT;
    state->new_ckey.format = ctrl->value;
    break;

  case V4L2_CID_STM_CKEY_R_CR_MODE:
    state->new_ckey.flags |= SCKCF_R_INFO;
    state->new_ckey.r_info = ctrl->value;
    break;

  case V4L2_CID_STM_CKEY_G_Y_MODE:
    state->new_ckey.flags |= SCKCF_G_INFO;
    state->new_ckey.g_info = ctrl->value;
    break;

  case V4L2_CID_STM_CKEY_B_CB_MODE:
    state->new_ckey.flags |= SCKCF_B_INFO;
    state->new_ckey.b_info = ctrl->value;
    break;

  case _deprecated_V4L2_CID_STM_CKEY_MINVAL:
    state->new_ckey.flags |= SCKCF_MINVAL;
    state->new_ckey.minval = ctrl->value;
    break;

  case _deprecated_V4L2_CID_STM_CKEY_MAXVAL:
    state->new_ckey.flags |= SCKCF_MAXVAL;
    state->new_ckey.maxval = ctrl->value;
    break;
  ...
  case V4L2_CID_STM_CKEY_BCb_MINVAL:
    {
    int shift = 16 - ((ctrl->id - V4L2_CID_STM_CKEY_RCr_MINVAL) * 4);
    state->new_ckey.flags |= SCKCF_MINVAL;
    state->new_ckey.minval &= ~(0x000000ff << shift);
    state->new_ckey.minval |= ((ctrl->value & 0xff) << shift);
    }
    break;
  ...
  case V4L2_CID_STM_CKEY_BCb_MAXVAL:
    {
    int shift = 16 - ((ctrl->id - V4L2_CID_STM_CKEY_RCr_MAXVAL) * 4);
    state->new_ckey.flags |= SCKCF_MAXVAL;
    state->new_ckey.maxval &= ~(0x000000ff << shift);
    state->new_ckey.maxval |= ((ctrl->value & 0xff) << shift);
    }
    break;
...
{
  switch (ctrl->id) {
  case V4L2_CID_STM_CKEY_ENABLE:    ctrl->value = state->old_ckey.enable; break;
  case V4L2_CID_STM_CKEY_FORMAT:    ctrl->value = state->old_ckey.format; break;
  case V4L2_CID_STM_CKEY_R_CR_MODE: ctrl->value = state->old_ckey.r_info; break;
  case V4L2_CID_STM_CKEY_G_Y_MODE:  ctrl->value = state->old_ckey.g_info; break;
  case V4L2_CID_STM_CKEY_B_CB_MODE: ctrl->value = state->old_ckey.b_info; break;
  case _deprecated_V4L2_CID_STM_CKEY_MINVAL: ctrl->value = state->old_ckey.minval; break;
  case _deprecated_V4L2_CID_STM_CKEY_MAXVAL: ctrl->value = state->old_ckey.maxval; break;
  case V4L2_CID_STM_CKEY_RCr_MINVAL: ctrl->value = (state->old_ckey.minval >> 16) & 0xff; break;
  case V4L2_CID_STM_CKEY_RCr_MAXVAL: ctrl->value = (state->old_ckey.maxval >> 16) & 0xff; break;
  case V4L2_CID_STM_CKEY_GY_MINVAL:  ctrl->value = (state->old_ckey.minval >>  8) & 0xff; break;
  case V4L2_CID_STM_CKEY_GY_MAXVAL:  ctrl->value = (state->old_ckey.maxval >>  8) & 0xff; break;
  case V4L2_CID_STM_CKEY_BCb_MINVAL: ctrl->value = (state->old_ckey.minval >>  0) & 0xff; break;
  case V4L2_CID_STM_CKEY_BCb_MAXVAL: ctrl->value = (state->old_ckey.maxval >>  0) & 0xff; break;
...
#define CHECK_STATE(ctrl, old_state, new_state) \
    ...
    /* color key */
    if (memcmp (&state->old_ckey, &state->new_ckey, sizeof (state->old_ckey))) {
      ...
      if (state->new_ckey.flags & SCKCF_ENABLE
          && (!(state->new_ckey.enable & VCSCEF_ENABLED)
              || ((state->new_ckey.enable & VCSCEF_ACTIVATE_BUFFER)
                  && !(state->old_ckey.enable & VCSCEF_ACTIVATE_BUFFER)))) {
        ...
        if (ret)
          return signal_pending (current) ? -ERESTARTSYS : -EBUSY;
      } else if (!(state->new_ckey.enable & VCSCEF_ACTIVATE_BUFFER)) {
        /* otherwise just set it (if not per buffer) */
        ret = stm_display_plane_set_control (device->pPlane,
                                             PLANE_CTRL_COLOR_KEY,
                                             (unsigned long) &state->new_ckey);
        if (ret)
          return signal_pending (current) ? -ERESTARTSYS : -EBUSY;
      }
