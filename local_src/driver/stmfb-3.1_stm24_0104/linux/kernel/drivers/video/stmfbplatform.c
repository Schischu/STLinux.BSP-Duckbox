...
static void stmfb_enumerate_modes(struct stmfb_info *i)
{
  ...
  for (n=0; n<STVTG_TIMING_MODE_COUNT; n++) {
    ...
    /* the pixel repeated modes are not to be set through any interface, they
       are used internally by the HDMI kernel thread. */
    if(mode->ModeParams.OutputStandards & (STM_OUTPUT_STD_TMDS_PIXELREP_2X
                                           | STM_OUTPUT_STD_TMDS_PIXELREP_4X))
      continue;
