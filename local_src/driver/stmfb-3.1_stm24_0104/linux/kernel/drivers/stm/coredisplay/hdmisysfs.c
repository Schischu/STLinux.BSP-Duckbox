/***********************************************************************
 *
 * File: linux/kernel/drivers/stm/coredisplay/hdmisysfs.c
...
static int enumerate_dvi_modes(struct stm_hdmi *hdmi, const stm_mode_line_t **display_modes)
{
  ...
  for (i=0; i<STVTG_TIMING_MODE_COUNT; i++) {
     ...
     */
    if((mode->ModeParams.OutputStandards & (STM_OUTPUT_STD_CEA861C | STM_OUTPUT_STD_VESA)) == 0)
      continue;

    /*
     * The pixel repeated modes are not available in DVI.
     */
    if((mode->ModeParams.OutputStandards & STM_OUTPUT_STD_TMDS_PIXELREP_2X) != 0)
      continue;
...
#if defined(__TDT__) 
static ssize_t show_hdmi_edid(struct device *dev, struct device_attribute *attr, char *buf)
{
  struct stm_hdmi *hdmi = dev_get_drvdata(dev);
