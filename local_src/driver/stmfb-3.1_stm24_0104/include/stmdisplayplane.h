...
/* Gives access to the background, Video 1&2, GDP 1...4, Cursor, etc. planes */
 ...
 *
 * Returns: -1 if the device lock cannot be obtained, otherwise the
 ...
 *   - Brightness (PLANE_CTRL_PSI_BRIGHTNESS)
 *   - Gain (PLANE_CTRL_GAIN)
 *   - IQI (PLANE_CTRL_IQI_CONFIG, etc.)
 *   - DEI (PLANE_CTRL_DEI_MODE, etc.)
 *
 * Returns: -1 if the device lock cannot be obtained, the control is not
 * supported or the value set is invalid, otherwise 0.
 *
 */
 ...
 * supported, otherwise 0 and the control's value is copied to the address
 * provided.
 *
 */
 ...
 *                                 int activate)
 *
 * Set the plane's depth in the plane stack of the specified output.
 *
 * If a plane can be attached to multiple outputs then the position
 * can be different on each output.
 * If the activate parameter is 0 then the change is only tested for validity,
 * the actual plane order is not changed.
 *
 * Returns: -1 if the device lock cannot be obtained, the plane is not usable
 * on the given output or the specified depth is invalid, otherwise 0.
 *
 */
 ...
 *                                 int *depth)
 *
 * Get the plane's depth in the plane stack of the specified output.
 *
 * If a plane can be attached to multiple outputs then the position
 * can be different on each output.
 *
 * Returns: -1 if the device lock cannot be obtained. It returns %false if
 * paramters are wrong, e.g. the plane is unknown, otherwise %true
 ...
 *
 * Get the current plane status flags. Status flags include, but are not
 * limited to:
 *   - STM_PLANE_STATUS_CONNECTED
 *   - STM_PLANE_STATUS_VISIBLE
 *   - STM_PLANE_STATUS_PAUSED
 *
 ...
 *
 * The plane will no longer contribute to the output's display and all
 * hardware processing on the plane will be stopped.
 *
 * Returns: -1 if the device lock cannot be obtained, otherwise 0.
 *
 */
 ...
 * fails for any reason, otherwise 0.
 *
 */
 ...
 *
 * Pause buffer queue processing.
 *
 * The buffer currently being processed by the hardware will continue to
 * be on the display, to produce a still image.
 *
 * If the bFlushQueue parameter is %true then the buffer queue is flushed,
 * causing any buffer completed callbacks to fire.
 * However caution is required, as it is permissible for the implementation
 * to split the processing of a single buffer into multiple queue entries.
 * This means that the completion callback for the buffer being processed
 * by the hardware may be called by the flush. Therefore it is not recommended
 * that the contents of flushed buffers be changed while paused.
 * The intended use of this facility is to be able to requeue buffers with
 * different parameters to avoid large user perceptible latencies, e.g.
 * switching from slow motion to normal playback.
 *
 * Returns: -1 if the device lock cannot be obtained, otherwise 0.
 *
 */
 ...
 *
 * Release the given plane instance.
 *
 * If this instance has locked the underlying plane's buffer queue, then
 * this lock is released and the queue flushed.
 *
 * Returns: -1 if the device lock cannot be obtained, otherwise 0.
 *
 * Note: The instance pointer is invalid after this call completes sucessfully.
 */
