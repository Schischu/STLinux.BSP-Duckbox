...
/* Gives access to system's output(s), plane(s) and blitter(s) */
 ...
 * Note: Currently, only STi7200 implements id == 1 (0: local, 1: remote)
 * all other SoCs have a device 0 only, as only STi7200 can be abstracted
 ...
 *
 * Outputs can be enumerated by increasing the outputID until the function
 * returns NULL. To find the intended output, enumerate them and use
 * stm_display_output_get_capabilities() until the right one is found.
 *
 * Returns: NULL if the device lock cannot be obtained or the output
 ...
 *
 * Blitters can be enumerated by increasing the blitterID until the function
 * returns NULL.
 *
 ...
 *
 * Note: This call can result in memory allocated from the OS heap being
