                                                ...
                                                unsigned int            Value)
{
    ...
    switch (Option)
    {
        ...
        case PLAY_OPTION_H264_VALIDATE_DPB_VALUES_AGAINST_PTS_VALUES:
    ...
    switch (Option)
    {
        ...
        case PLAY_OPTION_H264_FORCE_PIC_ORDER_CNT_IGNORE_DPB_DISPLAY_FRAME_ORDERING:
            *PlayerPolicy           = PolicyH264ForcePicOrderCntIgnoreDpbDisplayFrameOrdering;
            break;
        case PLAY_OPTION_H264_VALIDATE_DPB_VALUES_AGAINST_PTS_VALUES:
            *PlayerPolicy           = PolicyH264ValidateDpbValuesAgainstPTSValues;
            break;
        case PLAY_OPTION_H264_TREAT_TOP_BOTTOM_PICTURE_STRUCT_AS_INTERLACED:
...
HavanaStatus_t HavanaStream_c::SetPlayInterval (unsigned long long Start, unsigned long long End)
{
    ...
    PlayStart           = (Start == PLAY_TIME_NOT_BOUNDED) ? INVALID_TIME : Start;
    PlayEnd             = (End   == PLAY_TIME_NOT_BOUNDED) ? INVALID_TIME : End;

    STREAM_DEBUG("Setting play interval from %llx to %llx\n", Start, End);
    Status      = Player->SetPresentationInterval      (PlayerPlayback, PlayerStream, PlayStart, PlayEnd);
    if (Status != PlayerNoError)
    {
...
//    STREAM_DEBUG("\n");
    ...
    switch( Format )
    {
        ...
        default:
            STREAM_ERROR ("Request for unsupported decode buffer format (%d)\n", Format);
            return HavanaError;
    }
        ...
        ((PlayerEvent->Code & STREAM_SPECIFIC_EVENTS) != 0))
    {
        ...
        switch (PlayerEvent->Code)
        {
            case EventSourceSizeChangeManifest:
            {
                ...
                StreamEvent.u.size.width                = PictureWidth;
                StreamEvent.u.size.height               = PictureHeight;
