...
static int snd_card_pseudo_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_RESUME:
		err = PLAYER_ERRNO (pseudo->backend_ops->mixer_start_substream(
				    pseudo->backend_mixer, ppcm->substream_identifier));
		break;

	case SNDRV_PCM_TRIGGER_STOP:
	case SNDRV_PCM_TRIGGER_SUSPEND:
		err = PLAYER_ERRNO (pseudo->backend_ops->mixer_stop_substream(
				    pseudo->backend_mixer, ppcm->substream_identifier));
		break;
...
static int snd_card_pseudo_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	return PLAYER_ERRNO (pseudo->backend_ops->mixer_prepare_substream(
			     pseudo->backend_mixer, ppcm->substream_identifier));
}
				     ...
				     struct snd_pcm_hw_params *hw_params)
{
	...
	err = PLAYER_ERRNO (pseudo->backend_ops->mixer_setup_substream(
                        pseudo->backend_mixer, ppcm->substream_identifier,
			    &descriptor));
        if (err < 0) {
        	snd_card_pseudo_free_pages(substream);
...
static int snd_card_pseudo_hw_free(struct snd_pcm_substream *substream)
{
	...
	if (ppcm->backend_is_setup) {
		...
		err = PLAYER_ERRNO (pseudo->backend_ops->mixer_prepare_substream(
				    pseudo->backend_mixer, ppcm->substream_identifier));
		if (err < 0)
			return err;
	}
...
static int snd_card_pseudo_playback_open(struct snd_pcm_substream *substream)
{
	...
	err = PLAYER_ERRNO (pseudo->backend_ops->mixer_alloc_substream(
			    pseudo->backend_mixer, &ppcm->substream_identifier));
...
static int snd_card_pseudo_playback_close(struct snd_pcm_substream *substream)
{
	...
	err = PLAYER_ERRNO (pseudo->backend_ops->mixer_free_substream(
			    pseudo->backend_mixer, ppcm->substream_identifier));
	if (err < 0)
		return err;
...
static void snd_pseudo_mixer_update(struct snd_pseudo *pseudo)
{
	...
	err = PLAYER_ERRNO (pseudo->backend_ops->mixer_set_module_parameters(
...
static player_status_t snd_pseudo_default_backend_get_instance(int StreamId, component_handle_t* Classoid)
{
	 return PLAYER_NO_DEVICE;
}

static player_status_t snd_pseudo_default_backend_set_module_parameters(component_handle_t Classoid,
                                                     void *Data, unsigned int Size)
{
	return PLAYER_NO_ERROR;
}

static player_status_t snd_pseudo_default_backend_alloc_substream(component_handle_t Component, int *SubStreamId)
{
	return PLAYER_NO_DEVICE;
}
                                            ...
                                            struct alsa_backend_operations *alsa_backend_ops)
{
	...
	err = PLAYER_ERRNO (pseudo->backend_ops->mixer_get_instance(pseudo->room_identifier, &mixer));
        if (err != 0)
                return err;
