...
unsigned int Cycles;
    ...
    {
        while( PlaybackThreadRunning && !PcmPlayerSurfaceParameters[0].PeriodParameters.SampleRateHz )
        {
    ...
    if(ActualTopology.num_cards>1) {
        for(unsigned int n=1;n<ActualTopology.num_cards;n++) {
            memcpy(PcmPlayerMappedSamples[n],
                   ...
                   PcmPlayer[n]->SamplesToBytes(PcmPlayerSurfaceParameters[0].PeriodSize));
        }
    }
*/
    for(unsigned int n=0;n<ActualNumDownstreamCards;n++) {
...
{
ManifestorStatus_t Status;
unsigned int BufferSize = PcmPlayerSurfaceParameters[0].PeriodSize * PcmPlayerSurfaceParameters[0].NumPeriods;
...
bool RedundantUpdate = true;
    ...
    for(unsigned int n=0;n<ActualNumDownstreamCards;n++) 
    {
        NewSurfaceParameters[n].PeriodParameters.BitsPerSample = 32;
        NewSurfaceParameters[n].PeriodParameters.ChannelCount = LookupOutputNumberOfChannels(n);
        NewSurfaceParameters[n].PeriodParameters.SampleRateHz = NominalOutputSamplingFrequency;
        NewSurfaceParameters[n].ActualSampleRateHz = LookupOutputSamplingFrequency( n );
        NewSurfaceParameters[n].PeriodSize = NominalMixerGranuleSize;
	...
	if ( 0 != memcmp( &NewSurfaceParameters[n], &PcmPlayerSurfaceParameters[n],
			  sizeof( NewSurfaceParameters[n] ) ) )
	{
	    MIXER_DEBUG("Mismatch for #%d: %d-vs-%d  %d-vs-%d  %d-vs-%d  %d-vs-%d  %d-vs-%d  %d-vs-%d\n",
			n,
			NewSurfaceParameters[n].PeriodParameters.BitsPerSample,
			PcmPlayerSurfaceParameters[n].PeriodParameters.BitsPerSample,
			NewSurfaceParameters[n].PeriodParameters.ChannelCount,
			PcmPlayerSurfaceParameters[n].PeriodParameters.ChannelCount,
			NewSurfaceParameters[n].PeriodParameters.SampleRateHz,
			PcmPlayerSurfaceParameters[n].PeriodParameters.SampleRateHz,
			NewSurfaceParameters[n].ActualSampleRateHz,
			PcmPlayerSurfaceParameters[n].ActualSampleRateHz,
			NewSurfaceParameters[n].PeriodSize,
			PcmPlayerSurfaceParameters[n].PeriodSize,
			NewSurfaceParameters[n].NumPeriods,
			PcmPlayerSurfaceParameters[n].NumPeriods);
    ...
    for(unsigned int n=0;n<ActualNumDownstreamCards;n++) 
    {
        ...
        if (Status != PlayerNoError)
        {
            ...
            else
            {
                ...
                for(unsigned int m=0;m<ActualNumDownstreamCards;m++)
                {
                    NewSurfaceParameters[m] = PcmPlayerSurfaceParameters[m];
                    Status = PcmPlayer[m]->SetParameters(&NewSurfaceParameters[m]);
    ...
    for (unsigned int n=0;n<ActualNumDownstreamCards;n++)
    {
	PcmPlayerSurfaceParameters[n] = NewSurfaceParameters[n];
    }
    
    MIXER_DEBUG( "PcmPlayer parameters - NumPeriods %u PeriodSize %u\n",
                 PcmPlayerSurfaceParameters[0].NumPeriods, PcmPlayerSurfaceParameters[0].PeriodSize );
    
    if( PcmPlayerSurfaceParameters[0].PeriodSize != NominalMixerGranuleSize )
    {
        MIXER_ERROR("Invalid PCM player period size!\n");
        return PlayerError;
    }
...
unsigned int Mixer_Mme_c::LookupOutputSamplingFrequency( int dev_num )
{
	...
	MIXER_DEBUG("Output %d requires sampling frequency of %d (bypassed)\n", dev_num, Freq);
        return (Freq);
    }
    ...
    MIXER_DEBUG("Output %d requires sampling frequency of %d\n", dev_num, Freq);
    return Freq;
}
...
#undef _B
    ...
    {       
	    ...
	    ( ( OutputEncoding == PcmPlayer_c::BYPASS_AC3 ) &&
	      ( Clients[dev_num].Parameters.OriginalEncoding == AudioOriginalEncodingDdplus ) ) )
        {
...
{
PlayerStatus_t Status;
unsigned int PeriodSize = PcmPlayerSurfaceParameters[0].PeriodSize;;
